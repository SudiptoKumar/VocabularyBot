...
def test_cache_and_media() -> None:
    ...
    state["content_cache"][word.id] = rich().to_dict() | {"_content_version": SETTINGS.content_version, "_meaning_fingerprint": hashlib.sha256(word.meaning_bn.strip().encode("utf-8")).hexdigest()}
    assert word.id in app._enrichment_from_cache(state, [word])

    originals = (app.generate_card, app.ensure_audio, app.send_rich_message, app.save_state)
    try:
        ...
        app.generate_card, app.ensure_audio, app.send_rich_message, app.save_state = originals
...
def test_table_initial_capitalization() -> None:
    from rich_message import _table_words, _word_family_table
    assert _table_words([{"word": "high", "meaning": "of great vertical extent"}])[1] == ["High", "Of great vertical extent"]
    assert _word_family_table([{"word": "buying", "type": "noun"}])[1] == ["Buying", "Noun"]


def test_meaning_audit_catches_known_misalignment() -> None:
    from meaning_audit import MeaningAudit, apply_audits
    from dataclasses import replace
    words = load_all()
    tall = next(w for w in words if w.normalized_term == "tall")
    assert "কথা বলা" in tall.meaning_bn
    corrected = {tall.id: MeaningAudit(
        status="corrected",
        meaning_bn="লম্বা • উঁচু",
        confidence=0.99,
        reason="Source Bangla meaning belongs to talk, not tall",
        source_fingerprint="test",
        audited_at="test",
    )}
    fixed = apply_audits([tall], corrected)[0]
    assert fixed.meaning_bn == "লম্বা • উঁচু"



def test_content_cache_invalidates_when_verified_meaning_changes() -> None:
    import hashlib
    words = load_all()
    word = next(w for w in words if w.normalized_term == "tall")
    state = fresh_state(words)
    state["content_cache"][word.id] = rich().to_dict() | {
        "_content_version": SETTINGS.content_version,
        "_meaning_fingerprint": hashlib.sha256("wrong meaning".encode("utf-8")).hexdigest(),
    }
    assert word.id not in app._enrichment_from_cache(state, [word])


def test_audio_uses_streamable_rich_audio_block() -> None:
    words = load_all()
    ...
    assert audio_blocks[0]["audio"]["title"] == words[0].term[:64]
...
if __name__ == "__main__":
    ...
    test_meaning_audit_catches_known_misalignment()
    test_audio_uses_streamable_rich_audio_block()
    test_content_cache_invalidates_when_verified_meaning_changes()
    print("V1.2.0 SAFETY REGRESSIONS PASS")
