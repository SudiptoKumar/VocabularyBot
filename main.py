...
def _audit_meanings(state: dict, words: list[WordEntry]) -> tuple[dict[str, Any], list[WordEntry]]:
    ...
    # Do not silently publish a meaning that the auditor classified as uncertain.
    uncertain = [w for w in words if audits.get(w.id) and audits[w.id].status == "uncertain"]
    if uncertain:
        raise RuntimeError("Meaning verification uncertain for: " + ", ".join(w.term for w in uncertain))
