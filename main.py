...
from meaning_audit import audit_words, apply_audits
from telegram_client import TelegramAPIError, TelegramUnknownOutcomeError, extract_media_ids, preflight, send_rich_message
...
def _cache_enrichment(state: dict, enriched: dict[str, EnrichedWord], words: list[WordEntry]) -> None:
    cache = state.setdefault("content_cache", {})
    source_by_id = {w.id: w for w in words}
    for wid, content in enriched.items():
        ...
        source = source_by_id.get(wid)
        payload["_meaning_fingerprint"] = hashlib.sha256(str(source.meaning_bn if source else "").strip().encode("utf-8")).hexdigest()
        cache[wid] = payload
...
def _enrichment_from_cache(state: dict, words: list[WordEntry]) -> dict[str, EnrichedWord]:
    ...
    for word in words:
        ...
        if str(raw.get("_content_version", "")) != SETTINGS.content_version:
            continue
        expected_fp = hashlib.sha256(str(word.meaning_bn).strip().encode("utf-8")).hexdigest()
        if str(raw.get("_meaning_fingerprint", "")) != expected_fp:
            continue
...
def _audit_meanings(state: dict, words: list[WordEntry]) -> tuple[dict[str, Any], list[WordEntry]]:
    cache = state.setdefault("meaning_audit", {})
    audits = audit_words(words, cache)
    for wid, audit in audits.items():
        cache[wid] = {
            "status": audit.status,
            "meaning_bn": audit.meaning_bn,
            "confidence": audit.confidence,
            "reason": audit.reason,
            "source_fingerprint": audit.source_fingerprint,
            "audited_at": audit.audited_at,
        }
    # Uncertain audits are deliberately non-blocking. The safety rule is:
    #   confirmed wrong -> correct it
    #   confirmed right  -> keep source unchanged
    #   uncertain        -> keep source unchanged and continue the run
    uncertain = [w for w in words if audits.get(w.id) and audits[w.id].status == "uncertain"]
    if uncertain:
        logger.warning("Meaning verification uncertain; retaining original source meaning for: %s", ", ".join(w.term for w in uncertain))
    return audits, apply_audits(words, audits)


def _get_content(state: dict, words: list[WordEntry]) -> tuple[dict[str, EnrichedWord], list[WordEntry]]:
    _, verified_words = _audit_meanings(state, words)
    cached = _enrichment_from_cache(state, verified_words)
    missing = [w for w in verified_words if w.id not in cached]
    if missing:
        generated = enrich_words(missing)
        cached.update({wid: content for wid, content in generated.items()})
        _cache_enrichment(state, generated, verified_words)
    return cached, verified_words
...
def _publish_one(state: dict, run_id: str, word: WordEntry, enriched: EnrichedWord, cycle: int, position: int, total: int, dry_run: bool) -> bool:
    ...
    if SETTINGS.audio_enabled and not media.get("audio_file_id"):
        try:
...
def run(*, count: int | None = None, dry_run: bool = False) -> int:
    ...
    try:
        content, verified_chosen = _get_content(state, chosen)
    except RuntimeError as exc:
        reason = str(exc)
        logger.error(reason)
        for word in chosen:
            mark_failed(state, run_id, word.id, reason)
    ...
    chosen = verified_chosen
    # IMPORTANT: publish the audited WordEntry objects, not the original
    # dataset index. A confirmed meaning correction must reach both the card
    # and the rich post.
    verified_by_id = _index(chosen)
...
def self_test() -> int:
    ...
    sample_with_multi_bn = WordEntry(
        id=sample.id,
        term=sample.term,
        normalized_term=sample.normalized_term,
        pronunciation_bn=sample.pronunciation_bn,
        ipa=sample.ipa,
        meaning_bn="কেনা। | কিনতে।",
        source_numbers=sample.source_numbers,
        source_chapters=sample.source_chapters,
        sections=sample.sections,
        verb_hint=sample.verb_hint,
    ...
    # - Bangla meaning uses the requested "অর্থ⦂ ...।" post punctuation.
    # - Example is English followed directly by Bangla, with no বাংলা label.
    # - Only one intentional divider remains, before Example.
    # - Synonyms/Antonyms/Word Family are centered table captions.
    # - Common Collocations, Common Mistake, and Quiz are absent.
    ...
    table_blocks = [x for x in msg["blocks"] if x.get("type") == "table"]
    ...
    for tbl in table_blocks:
        cap = tbl.get("caption")
        if isinstance(cap, dict) and isinstance(cap.get("type"), str):
            captions.append(cap.get("text"))
    assert set(["Synonyms", "Antonyms", "Word Family"]).issubset(captions)
    paragraph_text = json.dumps(paragraph_blocks, ensure_ascii=False)
    all_blocks_json = json.dumps(msg["blocks"], ensure_ascii=False)
    assert "Common Collocations" not in paragraph_text
    assert "Common Mistake" not in paragraph_text
    assert "quiz" not in all_blocks_json.lower()
    ...
    assert any(
        ...
        and text[1] == "কেনা, কিনতে।"
        for text in bn_blocks
    )

    # Card Bangla punctuation regression: multiple meanings use bullets only.
    normalized_photo_bn = ""
    import re as _re
    raw_card_bn = "কেনা। | কিনতে।"
    _parts = _re.split(r"\s*(?:\||।|,|•)\s*", raw_card_bn)
    normalized_photo_bn = "  •  ".join([p.strip(" ,|।") for p in _parts if p.strip(" ,|।")])
    assert normalized_photo_bn == "কেনা  •  কিনতে"
    assert "।" not in normalized_photo_bn
    assert "," not in normalized_photo_bn
