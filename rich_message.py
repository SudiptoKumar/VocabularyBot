...
def _section_heading(text: str) -> dict[str, Any]:
    # Use the same bold paragraph treatment for every section title so there
    # are no emoji/decorative heading mismatches or native table captions.
...
def build_rich_message(
    ...
    *,
    ...
    if enriched.synonyms:
        blocks.append(_section_heading("Synonyms"))
        blocks.append(table(_table_words(enriched.synonyms)))

    if enriched.antonyms:
        blocks.append(_section_heading("Antonyms"))
        blocks.append(table(_table_words(enriched.antonyms)))

    if enriched.word_family:
        blocks.append(_section_heading("Word Family"))
        blocks.append(table(_word_family_table(enriched.word_family)))

    if enriched.collocations:
        blocks.append(_section_heading("Common Collocations"))
        blocks.append(list_block(enriched.collocations))

    if enriched.common_mistake_wrong and enriched.common_mistake_correct:
        blocks.append(_section_heading("Common Mistake"))
        blocks.append(paragraph(["✖  ", enriched.common_mistake_wrong]))
        blocks.append(paragraph(["✔  ", enriched.common_mistake_correct]))
